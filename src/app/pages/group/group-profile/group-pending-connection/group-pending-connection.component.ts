...
UntilDestroy();
@Component({
  selector: 'theLunchCircle-group-pending-connection',
  templateUrl: './group-pending-connection.component.html',
  styleUrls: ['./group-pending-connection.component.scss'],
  animations: [
    stagger80ms,
    scaleIn400ms,
    fadeInRight400ms,
...
export class GroupPendingConnectionComponent extends AppComponentBase implements OnInit, AfterViewInit {
  ...
  baselinePersonAdd=baselinePersonAdd;
  ...
  ngOnInit(): void {
    ...
    this.activatedRouter.queryParams.subscribe(async params => {
      ...
      this.blockUI.start();
      await this.apiService.get(environment.apiUrl + AppConsts.fetchGroupConnectionlist+'?groupId=' + this.groupId +'&statusType='+ 'Pending')
        .pipe(
          finalize(() => {
            this.blockUI.stop();
          })
  ...
  addConnection(user){
    ...
    objectData['body'] = "Do you want to approve this connection?"
    ...
    this.apiService.post(environment.apiUrl + AppConsts.updateStatusForGroupConnection, { 'groupConnectionList': list }).subscribe(async (result: any) => {
      if (result?.success) {
        ...
        this.subject$.next(this.userList);
        this.notificationService.open('Connection is approved successfully', 'success', 1000);
      }
      this.blockUI.stop();
    }, error => {
      this.blockUI.stop();
