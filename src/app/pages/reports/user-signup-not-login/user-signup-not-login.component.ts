...
@Component({
  ...
  templateUrl: './user-signup-not-login.component.html',
  styleUrls: ['./user-signup-not-login.component.scss'],
  animations: [
    stagger80ms,
    scaleIn400ms,
    fadeInRight400ms,
...
export class UserSignupNotLoginComponent extends AppComponentBase implements OnInit, AfterViewInit{
  ...
  dataSourceAct: MatTableDataSource<any> | null;
  ...
  ngOnInit() {
    ...
    this.data$.pipe(
      filter<any[]>(Boolean)
    ).subscribe(notLoginList => {
      this.notLoginList = notLoginList;
      this.dataSource.data = notLoginList;
    });
    this.searchCtrl.valueChanges.pipe(
      untilDestroyed(this)
    ).subscribe(value => this.onFilterChange(value));
    this.callFetchDataApi();
  }
  callFetchDataApi(){
    this.blockUI.start();
    this.apiService.post(environment.apiUrl+AppConsts.userSignupNotLogin,{}).subscribe(async (result:any) => {
  ...
  exportToExcel(){
    this.excelService.exportAsExcelFile(this.notLoginList, 'notSigninList');
  }
  refresh(){
  this.callFetchDataApi();
  }
