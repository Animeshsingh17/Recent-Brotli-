...
import { AddUserConnectionComponent } from '../add-user-connection/add-user-connection.component';

@UntilDestroy()
@Component({
  selector: 'theLunchCircle-user-connection',
...
export class UserConnectionComponent extends AppComponentBase implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRouter.queryParams.subscribe(async params => {
      ...
      this.blockUI.start();
      await this.apiService.get(environment.apiUrl + AppConsts.fetchUserConnectionInformation+'?userId=' + this.userId +'&statusType='+ 'Approved')
        .pipe(
          finalize(() => {
            this.blockUI.stop();
          })
  ...
  addConnection() {
    this.dialog.open(AddUserConnectionComponent, {
      data: this.userId
    }).afterClosed().subscribe(newUser => {
      if(newUser){
      this.friendList.unshift(new User(newUser));
      this.subject$.next(this.friendList);
