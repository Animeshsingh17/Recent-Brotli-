...
export class AdminNotificationComponent extends AppComponentBase implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.data$.pipe(
      filter<any[]>(Boolean)
    ).subscribe(adminNotificationList => {
      this.adminNotificationList = adminNotificationList;
      this.dataSource.data = adminNotificationList;
    });
    ...
    this.downloadAllAdminNotification();
  }
  ...
  downloadAllAdminNotification() {
    ...
    }).subscribe(async (adminNotificationList: any) => {
      this.blockUI.stop();
      if (adminNotificationList.success) {
        this.adminNotificationList=adminNotificationList.data.response;
        this.subject$.next(this.adminNotificationList);
      }
    }, error => {
      this.blockUI.stop();
      return error;
  ...
  exportToExcel(){
    this.excelService.exportAsExcelFile(this.adminNotificationList, 'adminNotificationList');
  }
  sendNotifications(){
    this.dialog.open(NotificationsComponent, {
      data: {
